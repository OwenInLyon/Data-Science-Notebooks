...
def get_scatter_plot(entered_site, selected_payload):
    ...
    if entered_site == 'ALL':
        ...
        title='Payload Mass vs. class colored by Booster Version at All Sites')
       
    else:
        # return the outcomes piechart for a selected site
        df = spacex_df[spacex_df['Launch Site'] == entered_site]
        fig = px.scatter(df[mask], x='Payload Mass (kg)', y='class', color="Booster Version Category", 
        title='Payload Mass vs. class colored by Booster Version at {}'.format(entered_site))
    return fig
# Run the app
